...
<section class="section-events">
    <div class="container">
        ...
        <div class="filter-card">
            <!-- Search bar -->
            <form action="{{ route('accueil') }}" method="GET" class="filter-search">
                ...
                <div class="d-flex flex-wrap gap-2 align-items-center">
                    ...
                    @foreach($categories as $cat)
                        <a href="{{ route('accueil') }}?categorie={{ $cat }}{{ $q ? '&q=' . $q : '' }}{{ $selectedDate ? '&date=' . $selectedDate : '' }}"
                           class="chip {{ $selectedCategorie == $cat ? 'active' : '' }}">
                            {{ ucfirst($cat) }}
                        </a>
                    @endforeach
                </div>
        ...
        @if($evenementsVedettes->isNotEmpty())
            <div class="text-center mt-4">
                <a href="{{ route('evenements.public') }}" class="btn-outline">Voir tous les événements <i class="bi bi-arrow-right ms-1"></i></a>
            </div>
        @endif
    </div>
</section>
