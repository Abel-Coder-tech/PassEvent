...
@section('title', 'Tous les événements - PassEvent')
...
<section class="ev-filters">
    <div class="container">
        <div class="filter-card-events">
            ...
            <form action="{{ route('evenements.public') }}" method="GET" class="filter-row">
                <div class="search-box">
                    <i class="bi bi-search"></i>
                    <input type="text" name="q" class="search-field" placeholder="Rechercher par nom, catégorie, lieu..." value="{{ $q ?? '' }}">
                </div>
                ...
                @if($q || $selectedCategorie || $selectedDate)
                    <a href="{{ route('evenements.public') }}" class="filter-btn-clear"><i class="bi bi-x-lg"></i></a>
                @endif
                <div class="filter-date-group">
                    <span class="filter-date-label"><i class="bi bi-funnel"></i></span>
                    <form action="{{ route('evenements.public') }}" method="GET" class="d-inline">
            ...
            <div class="filter-chips">
                <a href="{{ route('evenements.public') }}{{ $q ? '?q=' . $q : '' }}{{ $selectedDate ? ($q ? '&date=' . $selectedDate : '?date=' . $selectedDate) : '' }}"
                   class="chip {{ !$selectedCategorie ? 'active' : '' }}">
                ...
                @foreach($categories as $cat)
                    <a href="{{ route('evenements.public') }}?categorie={{ $cat }}{{ $q ? '&q=' . $q : '' }}{{ $selectedDate ? '&date=' . $selectedDate : '' }}"
                       class="chip {{ $selectedCategorie == $cat ? 'active' : '' }}">{{ ucfirst($cat) }}</a>
                @endforeach
            </div>
        </div>
    </div>
