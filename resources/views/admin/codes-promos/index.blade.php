...
@section('title', 'Codes Promo - PassEvent')
...
<div class="page-content">
    ...
    <div class="row g-3 mb-4">
        ...
        <div class="col-6 col-lg-3">
            <div class="metric-card" style="border-top-color: var(--vert);">
                ...
                <div class="metric-value" style="font-size: 1.5rem;">{{ number_format($stats['actifs']) }}</div>
            </div>
        </div>
        <div class="col-6 col-lg-3">
            <div class="metric-card" style="border-top-color: var(--teal);">
                ...
                <div class="metric-value" style="font-size: 1.5rem;">{{ number_format($stats['utilises']) }}</div>
            </div>
        </div>
        <div class="col-6 col-lg-3">
            <div class="metric-card" style="border-top-color: var(--danger);">
                ...
                <div class="metric-value" style="font-size: 1.1rem;">{{ number_format($stats['reduction_totale'], 0, ',', ' ') }} F</div>
            </div>
        </div>
    </div>
    ...
    <div class="panel-card mb-4">
        <div class="panel-card-body py-3">
            <form action="{{ route('admin.codes-promos.index') }}" method="GET" class="row g-2 align-items-end">
                <div class="col-12 col-md-3">
                    <label class="form-label fw-semibold" style="font-size: 0.78rem;">Rechercher un code</label>
                    <input type="text" name="q" class="form-control form-control-sm" placeholder="Ex: PROMO-..." value="{{ $q }}">
                </div>
                ...
                <div class="col-12 col-md-2">
                    <a href="{{ route('admin.codes-promos.index') }}" class="btn btn-secondary-custom btn-sm w-100" style="border-radius: 6px;">
    ...
    <div class="panel-card">
        <div class="panel-card-header">
            ...
            <span class="text-muted" style="font-size: 0.78rem;">{{ $codesPromos->total() }} code{{ $codesPromos->total() > 1 ? 's' : '' }}</span>
        </div>
        <div class="panel-card-body p-0">
            @if($codesPromos->count() > 0)
                        ...
                        <tbody>
                            @foreach($codesPromos as $code)
                                <tr>
                                    ...
                                    <td>
                                        <div style="font-size: 0.82rem;">{{ Str::limit($code->evenement->titre, 30) }}</div>
                                    </td>
                                    <td>
                                        @if($code->tarif)
                                            <span class="badge" style="background: var(--blanc-casse); color: var(--sombre); font-weight: 500;">{{ $code->tarif->getLabel() }}</span>
                                        @else
                                    ...
                                    <td>
                                        @if($code->type_reduction === 'pourcentage')
                                            <span class="badge" style="background: rgba(135,66,139,0.12); color: var(--violet); font-weight: 600;">-{{ $code->valeur_reduction }}%</span>
                                        @else
                                            <span class="badge" style="background: rgba(135,66,139,0.12); color: var(--violet); font-weight: 600;">-{{ number_format($code->valeur_reduction, 0, ',', ' ') }} F</span>
                                        @endif
                                    </td>
                                    <td>
                                        <div class="d-flex align-items-center gap-2">
                                            <span class="fw-bold" style="font-size: 0.85rem;">{{ $code->nb_utilisations }}</span>
                                            <div class="progress-bar-custom" style="width: 50px;">
                                                @php
                                                    $pct = $code->max_utilisations > 0 
                                                        ...
                                                        : 0;
                                                @endphp
                                                <div class="progress-bar-fill" style="width: {{ $pct }}%; background: {{ $pct >= 100 ? 'var(--danger)' : 'var(--vert)' }};"></div>
                                            </div>
                                            <span class="text-muted" style="font-size: 0.72rem;">/ {{ $code->max_utilisations ?? '∞' }}</span>
                                        </div>
                                    </td>
                                    <td>
                                        @if($code->date_expiration)
                                            <span style="font-size: 0.82rem;">{{ $code->date_expiration->format('d/m/Y') }}</span>
                                            @if($code->date_expiration->isPast() && $code->nb_utilisations == 0)
                                    ...
                                    <td>
                                        ...
                                        @elseif($code->date_expiration && $code->date_expiration->isPast())
                                    ...
                                    <td>
                                        <div style="font-size: 0.82rem;">{{ $code->created_at->format('d/m/Y') }}</div>
                                    </td>
                                    <td>
                                        <button type="button" class="btn btn-sm" style="border-radius: 6px; padding: 0.25rem 0.5rem; border: 1px solid var(--danger); color: var(--danger); background: transparent;" onclick="confirmDelete({{ $code->id }}, '{{ $code->code }}')" title="Supprimer">
                ...
                <div class="p-3">
                    {{ $codesPromos->appends(['q' => $q, 'evenement_id' => $selectedEvent, 'statut' => $statut])->links() }}
                </div>
            @else
...
<div class="modal fade" id="modalCreate" tabindex="-1">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content" style="border-radius: 12px; border: none;">
            ...
            <form action="{{ route('admin.codes-promos.store') }}" method="POST">
                @csrf
...
@section('scripts')
<script>
const tarifsData = @json($evenements->mapWithKeys(function($event) {
