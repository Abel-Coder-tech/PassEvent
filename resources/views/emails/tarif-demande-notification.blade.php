...
<head>
    ...
    <title>Nouvelle demande de modification de tarif</title>
    <style>
...
<body>
    <div class="container">
        ...
        <div class="body">
            <p>Bonjour PaxEvent,</p>
            <p>L'organisateur <strong>{{ $demande->user?->nom ?? '—' }}</strong> a soumis une demande de modification du prix d'un tarif.</p>
            <div class="infos">
                <div><strong>Événement :</strong> {{ $demande->evenement->titre }}</div>
                <div><strong>Tarif :</strong> {{ $demande->tarif->nom }}</div>
                <div><strong>Ancien prix :</strong> {{ number_format($demande->ancien_prix, 0, ',', ' ') }} F</div>
                <div><strong>Nouveau prix demandé :</strong> {{ number_format($demande->nouveau_prix, 0, ',', ' ') }} F</div>
            </div>
            <p>Connectez-vous à votre espace pour approuver ou refuser cette demande.</p>
            <p style="text-align: center;">
                <a href="{{ route('superadmin.demandes-modification-tarifs') }}" class="btn">Traiter la demande</a>
            </p>
