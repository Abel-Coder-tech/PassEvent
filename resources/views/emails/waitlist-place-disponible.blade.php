...
<head>
    ...
    <style>
        ...
        .infos { background: #f6f2fa; border-left: 4px solid #542680; padding: 14px 16px; border-radius: 8px; margin: 16px 0; }
        .infos div { font-size: 13px; color: #444; padding: 3px 0; }
        .infos strong { color: #3d1a5c; }
...
<body>
    <div class="container">
        ...
        <div class="body">
            <p>Bonjour {{ $ticket->nom_acheteur ?? 'cher participant' }},</p>
            <p>Bonne nouvelle : une place s'est libérée pour votre événement favori.</p>
            <div class="infos">
                <div><strong>Événement :</strong> {{ $evenement->titre }}</div>
                <div><strong>Tarif :</strong> {{ $ticket->nom_tarif }}</div>
                <div><strong>Montant :</strong> {{ number_format($ticket->montant, 0, ',', ' ') }} F</div>
            </div>
            <p>Votre place est <strong>réservée pour une durée limitée</strong>. Finalisez votre paiement dès maintenant pour la garantir :</p>
            <p style="text-align: center;">
                <a href="{{ $paiementUrl }}" class="btn">Procéder au paiement</a>
            </p>
            <p style="font-size: 13px; color: #6c757d;">Ce lien est valable 60 minutes. Si vous ne payez pas, la place sera remise en vente.</p>
