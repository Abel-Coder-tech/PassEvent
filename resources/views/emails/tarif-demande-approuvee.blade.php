...
<head>
    ...
    <title>Modification de tarif approuvée</title>
    <style>
        ...
        .header { background: linear-gradient(135deg, #2e7d4f, #1f5a38); color: #fff; padding: 28px 32px; text-align: center; }
        .header h1 { margin: 0; font-size: 20px; }
        .body { padding: 28px 32px; }
        .body p { font-size: 14px; color: #1d1d1f; margin: 0 0 12px; line-height: 1.6; }
        .infos { background: #f0f7ee; border-left: 4px solid #2e7d4f; padding: 14px 16px; border-radius: 8px; margin: 16px 0; }
        .infos div { font-size: 13px; color: #444; padding: 3px 0; }
        .infos strong { color: #1f5a38; }
        .btn { display: inline-block; background: #2e7d4f; color: #fff !important; padding: 12px 28px; border-radius: 10px; text-decoration: none; font-weight: 600; font-size: 14px; margin: 8px 0; }
...
<body>
    <div class="container">
        ...
        <div class="body">
            ...
            <p>Bonne nouvelle : votre demande de modification du prix d'un tarif a été <strong>approuvée</strong> !</p>
            <div class="infos">
                <div><strong>Événement :</strong> {{ $demande->evenement->titre }}</div>
                <div><strong>Tarif :</strong> {{ $demande->tarif->nom }}</div>
                <div><strong>Prix :</strong> {{ number_format($demande->ancien_prix, 0, ',', ' ') }} F → <strong>{{ number_format($demande->nouveau_prix, 0, ',', ' ') }} F</strong></div>
            </div>
            <p>Le nouveau prix est maintenant appliqué et visible sur la page de votre événement.</p>
